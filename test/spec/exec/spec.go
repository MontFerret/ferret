...
func S(expression string, expected any, desc ...string) spec.Spec {
	return spec.NewSpec(expression, desc...).Expect().Exec(assert.ShouldEqual, expected)
}
...
func ErrorAs(expression string, expected error, desc ...string) spec.Spec {
	return spec.NewSpec(expression, desc...).Expect().ExecError(assert.ShouldBeError, expected)
}
...
func Object(expression string, expected map[string]any, desc ...string) spec.Spec {
	s := spec.NewSpec(expression, desc...).Expect().Exec(assert.ShouldEqualJSON, expected)
...
func Items(expression string, expected ...any) spec.Spec {
	return spec.NewSpec(expression).Expect().Exec(assert.ShouldHaveSameItems, expected)
}

func Fn(expression string, assertion assert.Unary, desc ...string) spec.Spec {
	return spec.NewSpec(expression, desc...).Expect().Exec(assert.NewUnaryAssertion(assertion))
}
