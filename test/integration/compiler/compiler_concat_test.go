	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			for _, level := range []compiler.OptimizationLevel{compiler.None, compiler.Full} {
				...
				if stringAdds != 1 {
					t.Fatalf("%s: unexpected String-add count: got %d, want 1", level, stringAdds)
				}
			}
		})
	}
...
RETURN prefix + "b"`)
		...
		if got := countOpcodes(program.Bytecode, bytecode.OpAdd); got != 0 {
			t.Fatalf("%s: unexpected %s count: got %d, want 0", level, bytecode.OpAdd, got)
		}
		if got := countOpcodes(program.Bytecode, bytecode.OpConcat); got != 0 {
			t.Fatalf("%s: unexpected %s count: got %d, want 0", level, bytecode.OpConcat, got)
		}
		if level == compiler.None && countOpcodes(program.Bytecode, bytecode.OpAddConst) != 1 {
			t.Fatalf("%s: known string addition did not use %s", level, bytecode.OpAddConst)
		}
	}
}

func TestCompilerInvalidTemporalOrderingIsNotConstantFolded(t *testing.T) {
	program := compileWithLevel(t, compiler.Full, `RETURN 1s < "tomorrow"`)
