...
RETURN name`
	_, err := mustNewCompiler(t).Compile(t.Context(), source.New("duplicate_destructure.fql", src))
	if err == nil {
...
func TestForDestructuringDuplicateBindingDiagnostic(t *testing.T) {
	_, err := mustNewCompiler(t).Compile(t.Context(), source.NewAnonymous(`
...
func TestDestructuredBindingsAppearInDebugMetadata(t *testing.T) {
	prog, err := mustNewCompiler(t, compiler.WithDebugInfo()).Compile(t.Context(), source.NewAnonymous(`
