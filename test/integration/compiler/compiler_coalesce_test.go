...
func TestCoalesceMissingFallbackDiagnostic(t *testing.T) {
	input := "RETURN 1 ??"
	_, err := mustNewCompiler(t, compiler.WithOptimizationLevel(compiler.None)).Compile(t.Context(), source.NewAnonymous(input))
	if err == nil {
...
func TestCoalesceMissingFallbackDiagnosticIgnoresComments(t *testing.T) {
	_, err := mustNewCompiler(t, compiler.WithOptimizationLevel(compiler.None)).Compile(t.Context(), source.NewAnonymous("RETURN 1 + // ??"))
	if err == nil {
		t.Fatal("expected compilation to fail")
	}
