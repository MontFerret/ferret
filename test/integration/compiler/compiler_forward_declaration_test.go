...
func TestForwardDeclarationDiagnosticsNestedReturnlessLoop(t *testing.T) {
	diagnostics := compileDiagnostics(t, `
FOR outer IN [1] {
  FOR inner IN [outer] {
    LET value = later
...
func compileDiagnostics(t *testing.T, src string) []*diagpkg.Diagnostic {
	...
	_, err := mustNewCompiler(t, compiler.WithOptimizationLevel(compiler.OptimizationNone)).Compile(source.NewAnonymous(src))
	if err == nil {
