...
func TestHostFunctionRetryRecoverySuccess(t *testing.T) {
	for _, level := range []compiler.OptimizationLevel{compiler.None, compiler.Full} {
		...
		if got, want := callCount, 3; got != want {
			t.Fatalf("unexpected retry attempt count for %s: got %d, want %d", level, got, want)
		}
	}
}
...
func TestHostFunctionRetryExhaustionPropagatesFinalError(t *testing.T) {
	for _, level := range []compiler.OptimizationLevel{compiler.None, compiler.Full} {
		...
		if got, want := callCount, 2; got != want {
			t.Fatalf("unexpected propagated retry attempt count for %s: got %d, want %d", level, got, want)
		}
	}
}
...
func TestHostFunctionRetryFallbackFailurePropagates(t *testing.T) {
	for _, level := range []compiler.OptimizationLevel{compiler.None, compiler.Full} {
		...
		if got, want := callCount, 3; got != want {
			t.Fatalf("unexpected retry fallback attempt count for %s: got %d, want %d", level, got, want)
		}
	}
}
...
RETURN xs`, []any{11, 12}, "Grouped FOR retry should restart after cleanup without leaking partial results"),
		...
		if got, want := callCount, 3; got != want {
			t.Fatalf("unexpected grouped FOR retry attempt count for %s: got %d, want %d", level, got, want)
		}
	}
}
...
RETURN token`, "ok", "WAITFOR predicate should retry runtime failures and keep timeout handling separate"),
		...
		if got, want := callCount, 3; got != want {
			t.Fatalf("unexpected WAITFOR predicate retry attempt count for %s: got %d, want %d", level, got, want)
		}
	}
}
...
func TestWaitForEventRetryRecovery(t *testing.T) {
	for _, level := range []compiler.OptimizationLevel{compiler.None, compiler.Full} {
		...
		if got, want := successCalls, 3; got != want {
			t.Fatalf("unexpected WAITFOR EVENT retry attempt count for %s: got %d, want %d", level, got, want)
		}
		...
		if got, want := timeoutCalls, 1; got != want {
			t.Fatalf("unexpected WAITFOR EVENT timeout source call count for %s: got %d, want %d", level, got, want)
		}
	}
}
