...
func TestForStepDistinct(t *testing.T) {
	RunUseCases(t, []UseCase{
		CaseArray(
			`LET arr = [ 1, 2, 3, 4, 1, 3 ]
			FOR i = 0 WHILE i < LENGTH(arr) STEP i = i + 1
...
`, []any{
		...
		CaseArray(`
			...
			FOR i = 0 WHILE i < LENGTH(departments) STEP i = i + 1
				FOR j = 0 WHILE j < LENGTH(genders) STEP j = j + 1
		...
		CaseArray(`
			LET emptyArray = []
			FOR i = 0 WHILE i < LENGTH(emptyArray) STEP i = i + 1
		...
		CaseArray(`
			...
			FOR i = 0 WHILE i < LENGTH(departments) STEP i = i + 1
				...
				FOR j = 0 WHILE j < LENGTH(genders) STEP j = j + 1
					LET gender = genders[j]
					FOR k = 0 WHILE k < LENGTH(statuses) STEP k = k + 1
						RETURN DISTINCT { 
							department: dept, 
							gender: gender
						}
