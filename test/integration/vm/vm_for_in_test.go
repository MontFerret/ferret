...
`, []any{1, 1, 1, 1, 1}, "Should use ignored variable"),
		...
		Array(
			`
			RETURN FOR i IN [] RETURN i
				`,
			[]any{},
		),
		Array(
			`
			RETURN FOR i IN [1, 2, 3] RETURN i
				`,
			[]any{1, 2, 3},
		),
		Array(`
			RETURN FOR i, k IN [1, 2, 3] RETURN k`,
			[]any{0, 1, 2},
		),
		Array(`
			RETURN FOR i IN ['foo', 'bar', 'qaz'] RETURN i`,
			[]any{"foo", "bar", "qaz"},
		),
		Fn(`
			RETURN FOR i IN {a: 'bar', b: 'foo', c: 'qaz'} RETURN i`,
		...
		Fn(`
			RETURN FOR i, k IN {a: 'foo', b: 'bar', c: 'qaz'} RETURN k`,
			func(actual any) error {
				hashMap := make(map[string]bool)
		...
		Array(`
			RETURN FOR i IN [{name: 'foo'}, {name: 'bar'}, {name: 'qaz'}] RETURN i.name`,
			[]any{"foo", "bar", "qaz"},
		),
		Array(`
			RETURN FOR i IN { items: [{name: 'foo'}, {name: 'bar'}, {name: 'qaz'}] }.items RETURN i.name`,
			[]any{"foo", "bar", "qaz"},
		),
		Array(`
			RETURN FOR i IN GET_ITEMS()
				RETURN i
		`, []any{1, 2, 3}, "Should iterate over a function call source"),
		Array(`
			RETURN FOR order IN QUERY "/orders" IN @api
				RETURN order.id
		`, []any{"a", "b"}, "Should iterate over a query source with nested IN"),
		Array(`
			RETURN FOR order IN QUERY "/orders" IN @api WITH {
				query: {
					status: "open"
				}
			}
		...
		Array(`
			RETURN FOR item IN WAITFOR VALUE [1, 2]
				RETURN item
		`, []any{1, 2}, "Should iterate over a WAITFOR VALUE source"),
		Array(`
			RETURN FOR item IN MATCH "a" {"a" => [1], _ => [2]}
				RETURN item
