...
func TestCanonicalMathFunctions(t *testing.T) {
	cases := []spec.Spec{
		...
		Nil(`LET x=-0.0000000000000001 RETURN t::approx(math::expm1(x)/x,1,0.000001)`),
		S(`RETURN IS_FLOAT(math::clamp(5,0,10)) AND IS_INT(math::sign(1.5)) AND IS_INT(math::trunc(9007199254740993)) AND math::trunc(9007199254740993)==9007199254740993 AND IS_FLOAT(math::trunc(3.9))`, true),
