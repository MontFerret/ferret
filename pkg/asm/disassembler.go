...
func disasmLine(ip int, instr bytecode.Instruction, p *bytecode.Program, labels map[int]string, prev *bytecode.Instruction) string {
	...
	// Op R R Arg
	case bytecode.OpConcat, bytecode.OpAggregateReduce:
	...
	case bytecode.OpSourcePoint:
		out = fmt.Sprintf("%d: %s %d", ip, opcode, ops[0])
	case bytecode.OpAssertDestructure:
		out = fmt.Sprintf("%d: %s %s %s", ip, opcode, formatOperand(ops[0]), bytecode.DestructureMode(ops[1]))
