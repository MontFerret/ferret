...
func TestRange(t *testing.T) {
	Convey("Should return range of numbers", t, func() {
		out, err := arrays.Range(context.Background(), runtime.NewInt(1), runtime.NewInt(4))

		So(err, ShouldBeNil)
		So(out.String(), ShouldEqual, "[1,2,3,4]")

		out, err = arrays.Range(context.Background(),
			runtime.NewInt(1),
			runtime.NewInt(4),
			runtime.NewInt(2))
	...
	Convey("Should handle edge cases", t, func() {
		// Same start and end
		out, err := arrays.Range(context.Background(), runtime.NewInt(5), runtime.NewInt(5))

		So(err, ShouldBeNil)
		So(out.String(), ShouldEqual, "[5]")

		// Zero step should still work if only 2 args provided
		out, err = arrays.Range(context.Background(), runtime.NewInt(1), runtime.NewInt(3))
	...
	Convey("Should return error for invalid arguments", t, func() {
		// Too few arguments
		out, err := arrays.Range(context.Background(), runtime.NewInt(1))

		So(err, ShouldNotBeNil)
		So(out, ShouldEqual, runtime.None)

		// Non-numeric first argument
		out, err = arrays.Range(context.Background(), runtime.NewString("invalid"), runtime.NewInt(4))

		So(err, ShouldNotBeNil)
		So(out, ShouldEqual, runtime.None)

		// Non-numeric second argument
		out, err = arrays.Range(context.Background(), runtime.NewInt(1), runtime.NewString("invalid"))

		So(err, ShouldNotBeNil)
		So(out, ShouldEqual, runtime.None)

		// Non-numeric step argument
		out, err = arrays.Range(context.Background(), runtime.NewInt(1), runtime.NewInt(4), runtime.NewString("invalid"))
...
func TestRangeDirectionAndSafety(t *testing.T) {
	...
	t.Run("ascending negative endpoints", func(t *testing.T) {
		out, err := arrays.Range(ctx, runtime.NewInt(-3), runtime.NewInt(-1))
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("descending ranges", func(t *testing.T) {
		...
		for _, test := range tests {
			t.Run(test.name, func(t *testing.T) {
				out, err := arrays.Range(ctx, test.args...)
				if err != nil {
					t.Fatal(err)
				}
	...
	t.Run("zero step", func(t *testing.T) {
		out, err := arrays.Range(ctx, runtime.NewInt(1), runtime.NewInt(3), runtime.ZeroInt)
	...
	t.Run("step cannot advance", func(t *testing.T) {
		...
		out, err := arrays.Range(ctx, runtime.NewFloat(start), runtime.NewFloat(end), runtime.NewFloat(1))
		if !errors.Is(err, runtime.ErrInvalidArgument) {
			t.Fatalf("Range() error = %v, want ErrInvalidArgument", err)
		}
