...
func Minus(ctx runtime.Context, args ...runtime.Value) (runtime.Value, error) {
	...
	for idx, i := range args {
		...
		err = list.ForEach(ctx, func(c runtime.Context, value runtime.Value, _ runtime.Int) (runtime.Boolean, error) {
			h := value.Hash()
	...
	result := ctx.Alloc().Array(int(capacity))
