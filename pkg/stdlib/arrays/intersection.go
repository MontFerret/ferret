...
func Intersection(ctx runtime.Context, args ...runtime.Value) (runtime.Value, error) {
	return sections(ctx, args, len(args))
}

func sections(ctx runtime.Context, args []runtime.Value, count int) (runtime.Value, error) {
	...
	for _, i := range args {
		...
		err = list.ForEach(ctx, func(c runtime.Context, value runtime.Value, idx runtime.Int) (runtime.Boolean, error) {
			h := value.Hash()
