...
func TestValues(t *testing.T) {
	...
	Convey("When simple type attributes (same type)", t, func() {
		...
		for _, val := range values {
			if compareValues(val, runtime.NewInt(0)) == 0 {
				hasZero = true
			}
			if compareValues(val, runtime.NewInt(1)) == 0 {
				hasOne = true
			}
		}
	...
	Convey("When simple type attributes (different types)", t, func() {
		...
		for _, val := range values {
			...
			if compareValues(val, runtime.NewString("v2")) == 0 {
				hasString = true
			}
		}
		...
		for _, val := range values {
			if arr, ok := val.(*runtime.Array); ok {
				...
				if length == 2 {
					...
					if compareValues(val0, runtime.NewInt(0)) == 0 && compareValues(val1, runtime.NewInt(1)) == 0 {
						hasArr1 = true
					}
					if compareValues(val0, runtime.NewInt(2)) == 0 && compareValues(val1, runtime.NewInt(3)) == 0 {
						hasArr2 = true
					}
				}
			}
	...
	Convey("Result is independent on the source object (array)", t, func() {
		...
		So(compareValues(val, runtime.NewInt(0)), ShouldEqual, 0)
	})
	...
	Convey("When object has nil values", t, func() {
		...
		actualArray.ForEach(context.Background(), func(ctx context.Context, val runtime.Value, idx runtime.Int) (runtime.Boolean, error) {
			if compareValues(val, runtime.None) == 0 {
				foundNone = true
			}
			if compareValues(val, runtime.NewBoolean(false)) == 0 {
				foundBool = true
			}
			if compareValues(val, runtime.NewString("")) == 0 {
