...
func TestSort(t *testing.T) {
	Convey("Should sort asc", t, func() {
		...
		s, _ := collections.NewSorter(
			func(first collections.DataSet, second collections.DataSet) (int, error) {
				return first.Get(valVar).Compare(second.Get(valVar)), nil
			},
		...
		for idx, num := range numbers {
			So(res[idx].Get(valVar).Unwrap(), ShouldEqual, num)
		}
	})
	...
	Convey("Should sort asc with multiple sorters", t, func() {
		...
		s1, _ := collections.NewSorter(
			func(first collections.DataSet, second collections.DataSet) (int, error) {
				o1, _ := first.Get(valVar).(*values.Object).Get("one")
				o2, _ := second.Get(valVar).(*values.Object).Get("one")
		...
		s2, _ := collections.NewSorter(
			func(first collections.DataSet, second collections.DataSet) (int, error) {
				o1, _ := first.Get(valVar).(*values.Object).Get("two")
				o2, _ := second.Get(valVar).(*values.Object).Get("two")
