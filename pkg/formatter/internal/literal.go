...
func (f *literalFormatter) formatBooleanLiteral(ctx *fql.BooleanLiteralContext) {
	...
	f.p.write(f.config.FormatKeyword(ctx.BooleanLiteral().GetText()))
}
...
func (f *literalFormatter) formatNoneLiteral(ctx *fql.NoneLiteralContext) {
	...
	if ctx.Null() != nil {
		f.p.write(f.config.FormatKeyword(ctx.Null().GetText()))

		return
	}

	if ctx.None() != nil {
		f.p.write(f.config.FormatKeyword(ctx.None().GetText()))
	}
}
