...
func benchmarkUDFCallCancellationSafepoint(b *testing.B) {
	program := benchmarkCompileCancellationProgram(b, mustNewCompiler(b, compiler.WithOptimizationLevel(compiler.O0)), `
...
func benchmarkIteratorCancellationSafepoint(b *testing.B) {
	program := benchmarkCompileCancellationProgram(b, mustNewCompiler(b, compiler.WithOptimizationLevel(compiler.O0)), "FOR value IN 1..1024 RETURN value")
	benchmarkCancellationContexts(b, program, NewDefaultEnvironment())
}
...
func benchmarkHostCallCancellationSafepoint(b *testing.B) {
	...
	program := benchmarkCompileCancellationProgram(b, mustNewCompiler(b, compiler.WithOptimizationLevel(compiler.O0)), query.String())
...
func benchmarkReleaseCompiledCancellationSafepoint(b *testing.B) {
	program := benchmarkCompileCancellationProgram(b, mustNewCompiler(b, compiler.WithOptimizationLevel(compiler.O0)), "FOR value IN 1..128 RETURN value * 2")
	benchmarkCancellationContexts(b, program, NewDefaultEnvironment())
}
