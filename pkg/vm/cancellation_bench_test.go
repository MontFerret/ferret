...
func benchmarkIteratorCancellationSafepoint(b *testing.B) {
	program := benchmarkCompileCancellationProgram(b, compiler.New(compiler.WithOptimizationLevel(compiler.O0)), "FOR value IN 1..1024 RETURN value")
	benchmarkCancellationContexts(b, program, NewDefaultEnvironment())
}
...
func benchmarkHostCallCancellationSafepoint(b *testing.B) {
	...
	program := benchmarkCompileCancellationProgram(b, compiler.New(compiler.WithOptimizationLevel(compiler.O0)), query.String())
...
func benchmarkReleaseCompiledCancellationSafepoint(b *testing.B) {
	program := benchmarkCompileCancellationProgram(b, compiler.New(compiler.WithOptimizationLevel(compiler.O0)), "FOR value IN 1..128 RETURN value * 2")
	benchmarkCancellationContexts(b, program, NewDefaultEnvironment())
}

func benchmarkDebugCompiledCancellationSafepoint(b *testing.B) {
	program := benchmarkCompileCancellationProgram(b, compiler.New(compiler.WithDebugInfo()), "FOR value IN 1..128 RETURN value * 2")
	benchmarkCancellationContexts(b, program, NewDefaultEnvironment())
}
