...
func TestSessionFramesDoNotResolveLocationsAcrossFunctions(t *testing.T) {
	...
	points := []bytecode.DebugPoint{
		{ID: 4, PC: 2, Span: source.Span{Start: 0, End: 8}, FunctionID: 0},
		{ID: 7, PC: 4, Span: source.Span{Start: 9, End: 17}, FunctionID: 1},
		{ID: 2, PC: 6, Span: source.Span{Start: 18, End: 26}, FunctionID: 0},
	}
...
func TestSessionFramesUseGlobalPointWhenFunctionIdentityIsUnavailable(t *testing.T) {
	src := source.New("frames.fql", "RETURN 1\nRETURN 2")
	points := []bytecode.DebugPoint{
		{ID: 4, PC: 2, Span: source.Span{Start: 0, End: 8}, FunctionID: 0},
		{ID: 7, PC: 4, Span: source.Span{Start: 9, End: 17}, FunctionID: 1},
	...
	session, err := NewSession(Config{
		Execution: &fakeExecution{
			frames: []vm.DebugFrame{{Name: "unknown", FunctionID: -2, PC: 5}},
			status: vm.DebugExecutionPaused,
		},
		Values:      &fakeValueAccess{inner: vm.NewDebugValueAccess()},
		Services:    &fakeSessionServices{},
	...
	if len(frames) != 1 || frames[0].Location.Line != 2 {
		t.Fatalf("expected global fallback for unknown function identity: %#v", frames)
	}
}
