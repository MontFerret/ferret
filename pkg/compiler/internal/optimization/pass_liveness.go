...
func applyUnaryUseDef(opcode bytecode.Opcode, dst, src1 bytecode.Operand, collector *useDefCollector) bool {
	...
	case bytecode.OpIncr, bytecode.OpDecr:
		...
		collector.addDef(dst)
		return true
	case bytecode.OpCastBool, bytecode.OpNegate, bytecode.OpNot, bytecode.OpFlipPositive, bytecode.OpFlipNegative, bytecode.OpLength, bytecode.OpType, bytecode.OpFlatten, bytecode.OpDistinct, bytecode.OpExists, bytecode.OpAggregateReduce:
		collector.addUse(src1)
		collector.addDef(dst)
		return true
	default:
