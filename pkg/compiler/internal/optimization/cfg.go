...
func (bb *BasicBlock) IsTerminator() bool {
	...
	return op == bytecode.OpReturn ||
		op == bytecode.OpTailCall || op == bytecode.OpTailCall0 || op == bytecode.OpTailCall1 || op == bytecode.OpTailCall2 || op == bytecode.OpTailCall3 || op == bytecode.OpTailCall4 ||
		op == bytecode.OpJump || op == bytecode.OpJumpIfFalse || op == bytecode.OpJumpIfTrue || op == bytecode.OpJumpIfNone ||
		op == bytecode.OpJumpIfNe || op == bytecode.OpJumpIfNeConst || op == bytecode.OpJumpIfEq || op == bytecode.OpJumpIfEqConst ||
		op == bytecode.OpJumpIfMissingProperty || op == bytecode.OpJumpIfMissingPropertyConst || op == bytecode.OpIterNext
}
